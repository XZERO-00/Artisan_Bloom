...
export const Checkout = () => {
  ...
  const handleCheckout = async (e) => {
    ...
    toast.promise(orderPromise, {
      ...
      success: () => '🎉 Order placed! You will receive a confirmation shortly.',
      error: (err) => err?.message || 'Order failed. Please try again.',
    }, {
        ...
        >
          <form onSubmit={handleCheckout} className="bg-surface p-6 lg:p-8 rounded-[2rem] shadow-sm border border-black/5">
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-xs font-bold tracking-wide uppercase mb-2">Payment Method</label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  {[
                    { value: 'visa', label: 'VISA', style: 'font-bold italic text-blue-800 text-sm' },
                    { value: 'upi', label: 'UPI', style: 'font-bold text-[#FF7A00] text-sm tracking-wide' },
                    { value: 'gpay', label: 'GPay', style: 'font-bold text-gray-700 text-sm' },
                    { value: 'cod', label: 'COD', style: 'font-bold text-green-700 text-sm' },
                  ].map(pm => (
                    <label key={pm.value}
                      className={`flex flex-col items-center justify-center gap-1 border ${paymentMethod === pm.value ? 'border-primary ring-1 ring-primary' : 'border-primary/30'} rounded-xl p-3 cursor-pointer bg-background hover:border-primary transition-all min-h-[50px]`}>
                ...
                <div className="min-h-[60px]">
                  {paymentMethod === 'visa' && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="flex flex-col sm:flex-row gap-3 overflow-hidden">
                      <input type="text" required pattern="[0-9]{16}" placeholder="CARD NUMBER"
                        className="w-full sm:w-1/2 bg-background border border-primary/30 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#DFAA9D]/40 min-h-[44px]" />
                      <div className="flex gap-3 w-full sm:w-1/2">
                        <input type="text" required pattern="(0[1-9]|1[0-2])\/[0-9]{2}" placeholder="MM/YY"
                          className="w-1/2 bg-background border border-primary/30 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#DFAA9D]/40 min-h-[44px]" />
                        <input type="text" required pattern="[0-9]{3,4}" placeholder="CVC"
                          className="w-1/2 bg-background border border-primary/30 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#DFAA9D]/40 min-h-[44px]" />
                      </div>
                    </motion.div>
                  )}
                  {(paymentMethod === 'upi' || paymentMethod === 'gpay') && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="overflow-hidden">
                      <input type="text" name="upiId" required placeholder="Enter UPI ID (e.g. name@bank)"
                        className="w-full bg-background border border-primary/30 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#DFAA9D]/40 min-h-[44px]" />
                    </motion.div>
                  )}
                  {paymentMethod === 'cod' && (
              ...
              <div className="mt-4 text-center">
                <Link to="/cart" className="text-sm border-b border-textMain pb-0.5 hover:text-primary transition-colors">Go Back to Cart</Link>
              </div>
            </div>
          </form>
        </motion.div>
